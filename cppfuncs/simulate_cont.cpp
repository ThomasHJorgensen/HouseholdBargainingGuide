...
namespace sim {
    ...
    int update_power(int t, double power_lag, double love,double A_lag,double Aw_lag,double Am_lag,sim_struct* sim, sol_struct* sol, par_struct* par){
        ...
        if ((Vw_couple>=Vw_single) & (Vm_couple>=Vm_single)){
            ...
            if ((Vm_couple>=Vm_single)){ // woman want to leave
                ...
                double Vm_power = tools::interp_3d(par->grid_power,par->grid_love,par->grid_A, par->num_power,par->num_love,par->num_A, &sol->Vm_remain_couple[idx_sol], power,love,A_lag);
                ...
                // calculate value for woman at this level of power
                double Vw_power = tools::interp_3d(par->grid_power,par->grid_love,par->grid_A, par->num_power,par->num_love,par->num_A, &sol->Vw_remain_couple[idx_sol], power,love,A_lag);
